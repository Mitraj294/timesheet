...
const [projects, setProjects] = useState([]);
  ...
  const groupTimesheets = useMemo(() => {
    ...
    timesheets.forEach((timesheet) => {
      ...
      if (grouped[employeeId].hoursPerDay[formattedDate] !== undefined) {
        grouped[employeeId].hoursPerDay[formattedDate] += parseFloat(timesheet.totalHours);
      }
  ...
  {groupTimesheets.map((employee, index) => {
    ...
    return (
      <React.Fragment key={`${employee.employeeId || 'unknown'}-${index}`}>
          ...
          return (
            <tr key={`${employee.employeeId}-week-${weekIndex}`}>
              ...
              {week.map((day, dayIndex) => {
                ...
                return (
                  <td key={`${employee.employeeId}-${day.formatted}-${dayIndex}`} className='timesheet-cell'>
  ...
  {employee.details
    ...
    .map((entry) => (
      <div key={entry._id || `entry-${Math.random()}`} className='timesheet-entry'>
        ...
        <p><b>Employee Name:</b> {employee.name || 'N/A'}</p>
        <p><b>Date:</b> {entry.date || 'N/A'}</p>

        <p><b>Client:</b> {entry.clientId?.name || 'N/A'}</p>      
        <p><b>Project:</b> {entry.projectId?.name || 'N/A'}</p>    
        ...
        <p><b>Total Hours Worked:</b> {entry.totalHours || 'N/A'}</p>
