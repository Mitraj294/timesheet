...
export const downloadClients = async (req, res) => {
  try {
    ...
    const buffer = await generateClientTimesheetReport({ employerClients, clientProjects, employerEmployees, timesheetsGrouped });
...
export const sendClientsReportEmail = async (req, res) => {
  try {
    ...
    res.status(200).json({ message: 'Client timesheet report sent successfully!' });
  } catch (error) {
    console.error('Client report email error:', error);
    res.status(500).json({ message: `Failed to send client report: ${error.message}` });
  }
};
