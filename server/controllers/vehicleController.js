...
export const getVehicleById = async (req, res) => {
  try {
    ...
    const isEmployer =
      vehicle && vehicle.employerId.toString() === req.user.id.toString();
    const isEmployee =
      req.user.role === "employee" &&
      employee &&
      vehicle &&
      vehicle.employerId.toString() === employee.employerId.toString();

    if (!isEmployer && !isEmployee) {
      return res
        .status(404)
        .json({ message: "Vehicle not found or you do not have access." });
    }
    res.json(vehicle);
  } catch (err) {
...
export const createVehicle = async (req, res) => {
  try {
    ...
    if (!name || !hours) {
      return res
        .status(400)
        .json({ message: "Vehicle name and hours are required." });
    }
    const numericHours = Number(hours);
    if (isNaN(numericHours) || numericHours < 0) {
      return res
        .status(400)
        .json({ message: "Hours must be a valid non-negative number." });
    }
    const exists = await Vehicle.findOne({ name, employerId: req.user.id });
    if (exists) {
      return res
        .status(409)
        .json({ message: `A vehicle named '${name}' already exists.` });
    }
  ...
  try {
    ...
    if (!vehicle) {
      return res.status(404).json({
        message: "Vehicle not found or not associated with this employer.",
      });
    }
    if (name && name !== vehicle.name) {
      ...
      if (exists) {
        return res
          .status(409)
          .json({ message: `Another vehicle named '${name}' already exists.` });
      }
    }
    ...
    if (!updated) {
      return res
        .status(404)
        .json({ message: "Vehicle not found during update." });
    }
    res.json(updated);
  } catch (err) {
...
export const createVehicleReview = async (req, res) => {
  ...
  try {
    ...
    if (!dateReviewed) {
      return res
        .status(400)
        .json({ message: "Missing required review field: dateReviewed" });
    }
    if (!mongoose.Types.ObjectId.isValid(vehicleId)) {
      return res.status(400).json({ message: "Invalid vehicle ID format." });
    }
    ...
    if (role === "employer") {
      ...
      if (!employeeId || !mongoose.Types.ObjectId.isValid(employeeId)) {
        return res
          .status(400)
          .json({ message: "Valid employeeId is required." });
      }
    ...
    const isEmployer =
      role === "employer" &&
      vehicle.employerId.toString() === userId.toString();
    const isEmployee =
      role === "employee" &&
      employee.employerId &&
      vehicle.employerId.toString() === employee.employerId.toString();
...
export const getVehicleReviewsByVehicleId = async (req, res) => {
  try {
    ...
    let employee = null;
    if (req.user.role === "employee") {
      employee = await Employee.findOne({ userId: req.user.id }).select(
        "employerId",
      );
      if (!employee || !employee.employerId) {
        return res.status(403).json({ message: "Access denied." });
    ...
    if (!isEmployer && !isEmployee) {
      return res.status(404).json({
        message: "Vehicle not found or you do not have access to its reviews.",
      });
    }
    ...
    res
      .status(500)
      .json({ message: "Server error while fetching vehicle reviews" });
  }
};
...
export const getReviewById = async (req, res) => {
  try {
    ...
    const review = await VehicleReview.findById(reviewId).populate(
      "vehicle",
      "name wofRego employerId",
    );
    let employee = null;
    if (req.user.role === "employee") {
      employee = await Employee.findOne({ userId: req.user.id }).select(
    ...
    const isEmployer =
      review &&
      review.vehicle &&
      review.vehicle.employerId &&
      review.vehicle.employerId.toString() === req.user.id.toString();
    const isEmployee =
      req.user.role === "employee" &&
      employee &&
      review &&
      review.vehicle &&
      review.vehicle.employerId &&
      review.vehicle.employerId.toString() === employee.employerId.toString();
...
export const getVehicleWithReviews = async (req, res) => {
  try {
    ...
    const isEmployer =
      vehicle &&
      vehicle.employerId &&
      vehicle.employerId.toString() === req.user.id.toString();
    const isEmployee =
      req.user.role === "employee" &&
      employee &&
      vehicle &&
    ...
    res
      .status(500)
      .json({ message: "Server error while fetching vehicle and reviews" });
  }
};
...
export const updateReview = async (req, res) => {
  try {
    ...
    const isReviewOwner =
      ...
      review &&
      review.employeeId &&
      review.employeeId._id.toString() === employee?._id.toString();
    if (!review || !review.vehicle || (!isEmployer && !isReviewOwner)) {
      return res.status(403).json({ message: "Access denied." });
    }
    const { vehicle, employeeId, ...updateData } = req.body;
    const updated = await VehicleReview.findByIdAndUpdate(
      reviewId,
      updateData,
      { new: true, runValidators: true },
    )
...
const generateReviewFilename = (review, extension) => {
  const vehicleName =
    review.vehicle?.name?.replace(/\s+/g, "_") || "UnknownVehicle";
  const employeeName =
    review.employeeId?.name?.replace(/\s+/g, "_") || "UnknownEmployee";
  const reviewDate = review.dateReviewed
    ? new Date(review.dateReviewed).toISOString().split("T")[0]
    : "UnknownDate";
...
export const downloadReviewReport = async (req, res) => {
  ...
  try {
    ...
    const filename = generateReviewFilename(
      review,
      format === "pdf" ? "pdf" : "xlsx",
    );
    if (format === "pdf") {
      ...
      doc
        .fontSize(16)
        .font("Helvetica-Bold")
        .text("Vehicle Review Report", { align: "center" });
      doc.moveDown(2);
      doc.fontSize(12).font("Helvetica-Bold");
      doc
        .text(`Vehicle: `, { continued: true })
        .font("Helvetica")
        .text(`${review.vehicle?.name || "N/A"}`);
      doc
        .text(`WOF/Rego: `, { continued: true })
        .font("Helvetica")
        .text(`${review.vehicle?.wofRego || "N/A"}`);
      doc.moveDown(0.5);
      doc
        .text(`Date Reviewed: `, { continued: true })
        .font("Helvetica")
        .text(
          `${review.dateReviewed ? new Date(review.dateReviewed).toLocaleDateString() : "N/A"}`,
        );
      doc
        .text(`Employee: `, { continued: true })
        .font("Helvetica")
        .text(`${review.employeeId?.name || "N/A"}`);
      doc.moveDown(0.5);
      doc
        .text(`Hours Used: `, { continued: true })
        .font("Helvetica")
        .text(`${review.hours ?? "N/A"}`);
      doc.moveDown(1);
      ...
      doc
        .font("Helvetica")
        .text(review.notes || "N/A", { width: 410, align: "justify" });
      doc.end();
    } else {
      ...
      const data = [
        ...
        {
          field: "Vehicle Checked",
          value: review.vehicleChecked ? "Yes" : "No",
        },
    ...
    if (!res.headersSent) {
      res
        .status(500)
        .json({ message: "Internal server error generating report" });
    } else {
      res.end();
    }
  }
...
export const sendReviewReportByClient = async (req, res) => {
  ...
  if (!["pdf", "excel"].includes(format)) {
    return res
      .status(400)
      .json({ message: 'Invalid format specified. Use "pdf" or "excel".' });
  }
  try {
    const review = await VehicleReview.findById(reviewId)
      .populate("vehicle", "name wofRego employerId")
    ...
    let buffer;
    let filename = generateReviewFilename(
      review,
    ...
    res
      .status(200)
      .json({ message: "Review report sent successfully via email." });
  } catch (error) {
    res.status(500).json({
      message: `Failed to send review report via email: ${error.message}`,
    });
  }
};
...
export const downloadVehicleReport = async (req, res) => {
  try {
    ...
    summarySheet.getCell("A1").value =
      `Vehicle Summary Report: ${vehicle.name}`;
    ...
    if (reviews.length === 0) {
      ...
      historySheet.getCell("A2").value =
        "No reviews found for this vehicle in the specified date range.";
      ...
      reviews.forEach((r) => {
        historySheet.addRow({
          date: r.dateReviewed
            ? new Date(r.dateReviewed).toLocaleDateString()
            : "N/A",
      ...
      historySheet.getColumn("notes").alignment = {
        wrapText: true,
        vertical: "top",
      };
    }
    ...
    if (!res.headersSent) {
      res
        .status(500)
        .json({ message: "Error generating vehicle Excel report" });
    } else {
      res.end();
    }
  }
...
export const sendVehicleReportByEmail = async (req, res) => {
  try {
    ...
    if (!vehicleId || !email) {
      return res
        .status(400)
        .json({ message: "Vehicle ID and Email are required." });
    }
    const start = startDate ? new Date(startDate) : null;
    const end = endDate ? new Date(endDate) : null;
    if (start && isNaN(start.getTime()))
    ...
    res
      .status(200)
      .json({ message: "Vehicle report sent successfully via email." });
  } catch (error) {
    res.status(500).json({
      message: `Failed to send review report via email: ${error.message}`,
    });
...
export const downloadAllVehiclesReport = async (req, res) => {
  try {
    ...
    if (!vehicles || vehicles.length === 0) {
      return res
        .status(404)
        .json({ message: "No vehicles found in the system." });
    }
    const workbook = new ExcelJS.Workbook();
    workbook.creator = "Vehicle Management System";
    workbook.created = new Date();
    ...
    if (!res.headersSent) {
      res.status(500).json({
        message: "Internal server error generating all vehicles report",
      });
    } else {
      res.end();
    }
...
export const sendAllVehiclesReportByEmail = async (req, res) => {
  try {
    ...
    if (!email) {
      return res
        .status(400)
        .json({ message: "Recipient email address is required" });
    }
    const start = startDate ? new Date(startDate) : null;
    const end = endDate ? new Date(endDate) : null;
    if (start && isNaN(start.getTime()))
    ...
    if (!vehicles || vehicles.length === 0) {
      return res
        .status(404)
        .json({ message: "No vehicles found. Email not sent." });
    }
    const columns = [
      { header: "Vehicle Name", key: "vehicleName", width: 25 },
      { header: "Current Hours", key: "hours", width: 15 },
    ...
    res
      .status(200)
      .json({ message: "All vehicles report sent successfully via email!" });
  } catch (error) {
    res.status(500).json({
      message: `Failed to send all vehicles report email. ${error.message}`,
    });
  }
};

// Factory export for testing (mockable dependencies)
const vehicleControllerFactory = (deps) => ({
  getVehicles: (req, res) => getVehicles(req, res),
  getVehicleById: (req, res) => getVehicleById(req, res),
  createVehicle: (req, res) => createVehicle(req, res),
  updateVehicle: (req, res) => updateVehicle(req, res),
  deleteVehicle: (req, res) => deleteVehicle(req, res),
  // Add other controller methods as needed
});
export default vehicleControllerFactory;
