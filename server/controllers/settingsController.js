  ...
  try {
    ...
    for (const day of DAYS_OF_WEEK_LOWERCASE) {
      ...
      if (storedUtcDate instanceof Date) {
        ...
        const scheduledMomentInEmployerTZ = moment.tz(storedUtcDate, employerTimezone);
        if (scheduledMomentInEmployerTZ.isBefore(nowInEmployerTZ)) {
          const timeHHMM = scheduledMomentInEmployerTZ.format('HH:mm');
          const nextOccurrenceUTC = calculateNextScheduledUTC(day, timeHHMM, employerTimezone);
...
export const updateEmployerSettings = async (req, res, next) => {
  ...
  try {
    ...
    const employerTimezone = settingsToUpdate.timezone || currentSettings.timezone || 'UTC';
    ...
    if (clientGlobalNotificationTimes) {
      for (const day of DAYS_OF_WEEK_LOWERCASE) {
        ...
        processedGlobalNotificationTimes[day] = calculateNextScheduledUTC(day, localTimeHHMM, employerTimezone);
      }
    ...
    if (updatedSettings.globalNotificationTimes && updatedSettings.timezone) {
      for (const dayOfWeek of DAYS_OF_WEEK_LOWERCASE) {
        const newScheduledTimeUTC = updatedSettings.globalNotificationTimes[dayOfWeek];
        if (newScheduledTimeUTC instanceof Date) {
          ...
          for (const notification of notificationsToReschedule) {
            if (!notification.scheduledTimeUTC || notification.scheduledTimeUTC.getTime() !== newScheduledTimeUTC.getTime()) {
