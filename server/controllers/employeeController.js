...
export const getEmployees = async (req, res) => {
  ...
  try {
    ...
    if (req.user.role === 'employer') {
      employees = await Employee.find({ employerId: req.user.id }).sort({ name: 1 });
...
export const addEmployee = async (req, res) => {
  try {
    ...
    if (!name || !email || !employeeCode || wage === undefined || wage === null) {
...
export const batchUpdateEmployeeNotificationPreferences = asyncHandler(async (req, res) => {
  ...
  try {
    const bulkOps = updates.map(update => {
      ...
      return {
        updateOne: {
          filter: { _id: update.employeeId, employerId: employerIdForFilter },
