...
export const seedNotifications = async () => {
  ...
  try {
    ...
    const employerUser = await User.findOne({ _id: employerId }).select('email name').lean();
    if (!employerUser || !employerUser.email) {
      console.error(`Employer user not found or missing email for employerId: ${employerId}`);
      return;
    }

    const employees = await Employee.find({ employerId }).select('name email').lean();
    ...
    const scheduledTimeUTC = now.clone().hour(10).minute(0).second(0).millisecond(0).utc().toDate();
      ...
      for (const project of projects) {
        for (const client of clients) {
          ...
          if (documentsToInsert.length === batchSize) {
            ...
            console.log(`Inserted batch of ${documentsToInsert.length} notifications.`);
            documentsToInsert = [];
          }
        }
      }
    ...
    console.log(`Successfully inserted notifications for all employee/project/client combinations for today.`);
