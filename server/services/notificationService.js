...
const MAX_NOTIFICATIONS_PER_RUN =
  parseInt(process.env.MAX_NOTIFICATIONS_PER_RUN, 10) || 500;
  ...
  try {
    while (totalProcessedThisRun < MAX_NOTIFICATIONS_PER_RUN) {
      ...
      if (batchesProcessedThisRun === 1) {
        console.log(
          `[NotificationService] Cron job triggered at ${overallStartTime.toISOString()} and found work. Will process in batches of ${BATCH_SIZE}.`,
        );
      }
      const internalBatchStartTime = moment();
      console.log(
        `[NotificationService] Processing internal batch ${batchesProcessedThisRun}: Found ${pendingNotifications.length} notifications.`,
      );
      ...
      const emailProcessingPromises = pendingNotifications.map(
        async (notification) => {
          try {
            ...
            console.error(
              `[NotificationService] Failed to process email for notification ID ${notification._id} to ${notification.recipientEmail}. Error: ${emailError.message}`,
            );
            ...
            try {
              ...
              console.error(
                `[NotificationService] CRITICAL: Failed to save notification status for ID ${notification._id} after processing. Error: ${saveError.message}`,
              );
            }
          }
          return notification;
      ...
      console.log(
        `[NotificationService] All ${pendingNotifications.length} email sending tasks in internal batch ${batchesProcessedThisRun} have been initiated.`,
      );
      ...
      const internalBatchDurationMs = internalBatchEndTime.diff(
        internalBatchStartTime,
      );
      console.log(
        `[NotificationService] Internal batch ${batchesProcessedThisRun} complete in ${internalBatchDurationMs}ms. Processed: ${pendingNotifications.length}. Sent: ${currentBatchSuccessCount}. Failed: ${currentBatchFailureCount}.`,
      );
    ...
    if (totalProcessedThisRun > 0) {
      ...
      console.log(
        `[NotificationService] Overall processing for this run complete in ${overallDurationMs}ms. Total processed: ${totalProcessedThisRun} in ${batchesProcessedThisRun} internal batch(es). Total sent: ${totalSentThisRun}. Total failed: ${totalFailedThisRun}.`,
      );
    }
    if (totalProcessedThisRun >= MAX_NOTIFICATIONS_PER_RUN) {
      console.warn(
        `[NotificationService] Reached MAX_NOTIFICATIONS_PER_RUN (${MAX_NOTIFICATIONS_PER_RUN}). More notifications might be pending for the next run.`,
      );
    }
  } catch (error) {
    console.error(
      "[NotificationService] General error during processing pending notifications:",
      error,
    );
  }
};
...
export const startNotificationService = () => {
  ...
  if (cron.validate(cronSchedule)) {
    ...
    console.log(
      `[NotificationService] Notification service started. Will run with pattern "${cronSchedule}".`,
    );
  } else {
    console.error(
